import type { ScheduleWithAvailabilitiesForWeb } from '../../libraries/index.ts';
export declare const QUERY_KEY = "user-schedule";
declare const useClientSchedule: (id?: string) => {
    isLoading: boolean;
    error: Error | null;
    data: import('../../types/index.ts').ApiSuccessResponse<ScheduleWithAvailabilitiesForWeb> | undefined;
};
export default useClientSchedule;
//# sourceMappingURL=useClientSchedule.d.ts.map